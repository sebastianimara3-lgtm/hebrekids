...
// MODIFICACIÓN: agrega el banner remoto al inicio
// Solo hay que agregar estas líneas al HomeScreen existente

// ── PASO 1: Agregar estos imports al inicio del archivo ──────────
// import { obtenerBanner } from '../services/banner';
// import BannerModal from '../components/BannerModal';

// ── PASO 2: Agregar este estado dentro del componente ────────────
// const [banner, setBanner] = useState(null);

// ── PASO 3: Agregar esta llamada dentro del useEffect principal ──
// obtenerBanner().then(b => setBanner(b));

// ── PASO 4: Agregar el componente al final del JSX ───────────────
// <BannerModal banner={banner} onCerrar={() => setBanner(null)} />

// ════════════════════════════════════════════════════════════════
// HOMESCREEN COMPLETO CON BANNER INTEGRADO
// ════════════════════════════════════════════════════════════════
...
import { obtenerBanner }  from '../services/banner';      // ← NUEVO
import BannerModal        from '../components/BannerModal'; // ← NUEVO
...
export default function HomeScreen({ navigation }) {
  ...
  const [banner, setBanner] = useState(null); // ← NUEVO

  const flotacion = useSharedValue(0);
  const pulso     = useSharedValue(1);

  useEffect(() => {
    // Cargar progreso y racha
    ...
    // ── NUEVO: Cargar banner remoto ──────────────────────────────
    // Se ejecuta en paralelo, no bloquea la carga del juego
    obtenerBanner().then(b => {
      if (b) setBanner(b);
    });
    // ────────────────────────────────────────────────────────────
  ...
  return (
    <View style={styles.contenedor}>
      ...
      {/* ── BANNER REMOTO ── */}
      {/* Se muestra encima de todo cuando hay un banner activo en Firebase */}
      <BannerModal
        banner={banner}
