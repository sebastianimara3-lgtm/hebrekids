      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.grid}>
          {NIVELES_DATA.map(nivel => {
            const prog = getNivelProg(nivel.id);
            return (
              <TarjetaNivel
                key={nivel.id}
                nivel={nivel}
                desbloqueado={prog.desbloqueado}
...
const styles = StyleSheet.create({
  ...
  botonJugar:{marginHorizontal:16,marginTop:16,backgroundColor:'#FFE566',borderRadius:20,paddingVertical:16,alignItems:'center',elevation:10},
