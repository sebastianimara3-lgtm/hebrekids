...
const TarjetaNivel=({nivel,estrellas,onPress})=>{
  ...
  const press=()=>{escala.value=withSequence(withSpring(0.93,{damping:6}),withSpring(1.0,{damping:8}));setTimeout(()=>onPress(nivel),150);};
  return(
    <Animated.View style={[styles.tarjeta,estiloAnim]}>
      <TouchableOpacity onPress={press} activeOpacity={0.9} style={styles.tarjetaTouch}>
        ...
        <View style={styles.estrellasRow}>{[1,2,3].map(i=><Text key={i} style={{fontSize:16,opacity:i<=estrellas?1:0.2}}>⭐</Text>)}</View>
        <View style={styles.barraWrap}><View style={[styles.barraFill,{width:`${(estrellas/3)*100}%`,backgroundColor:nivel.color}]}/></View>
      </TouchableOpacity>
    </Animated.View>
  );
};
...
export default function HomeScreen({navigation}){
  const [estado,setEstado]=useState(null),[racha,setRacha]=useState(0);
  const flotacion=useSharedValue(0),pulso=useSharedValue(1);
  useEffect(()=>{
  ...
  useEffect(()=>{const u=navigation.addListener('focus',()=>cargarEstadoInicial().then(e=>{setEstado(e);setRacha(e.racha.diasConsecutivos);}));return u;},[navigation]);
    ...
    <View style={styles.contenedor}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.hud}>
          <View style={styles.rachaBadge}><Text style={styles.rachaIcono}>🔥</Text><Text style={styles.rachaNum}>{racha}</Text><Text style={styles.rachaLabel}>días</Text></View>
          <View style={styles.puntosBadge}><Text style={{fontSize:18}}>⭐</Text><Text style={styles.puntosNum}>{estado?.progreso?.puntosTotal||0}</Text></View>
          <View style={styles.gemasBadge}><Text style={{fontSize:18}}>💎</Text><Text style={styles.gemasNum}>{Math.floor((estado?.progreso?.puntosTotal||0)/10)}</Text></View>
        </View>
        ...
        <Animated.View style={[styles.mascotaWrap,estiloMascota]}>
          <View style={styles.bocadillo}><Text style={styles.bocadilloTexto}>¡Hola! ¿Listo para{'\n'}aprender hebreo? 🎉</Text></View>
          <Text style={styles.mascotaCara}>🦊</Text>
        </Animated.View>
        <Text style={styles.seccionTitulo}>🗺️ Tu camino</Text>
        <View style={styles.grid}>
          {NIVELES_DATA.map(n=><TarjetaNivel key={n.id} nivel={n} estrellas={getProg(n.id).estrellas} onPress={x=>navigation.navigate(x.screen)}/>)}
        </View>
        <Animated.View style={estiloBoton}>
          <TouchableOpacity style={styles.botonJugar} activeOpacity={0.9} onPress={()=>navigation.navigate('Level1')}>
            <Text style={styles.botonJugarTexto}>▶ ¡JUGAR AHORA!</Text>
            <Text style={styles.botonJugarSub}>{racha>0?`¡Tu racha de ${racha} días te espera! 🔥`:'¡Empezá tu aventura! 🚀'}</Text>
          </TouchableOpacity>
        </Animated.View>
        <View style={styles.navInferior}>
          {[{i:'🏠',l:'Inicio',a:true},{i:'🗺️',l:'Mapa'},{i:'🐾',l:'Mascota'},{i:'🏆',l:'Logros'}].map(item=>(
            <View key={item.l} style={styles.navItem}><Text style={styles.navIcono}>{item.i}</Text><Text style={[styles.navLabel,item.a&&styles.navLabelActivo]}>{item.l}</Text></View>
          ))}
        </View>
      </ScrollView>
    </View>
...
const styles=StyleSheet.create({
  contenedor:{flex:1,backgroundColor:'#1a1060'},scroll:{paddingBottom:20},
  ...
  puntosBadge:{flexDirection:'row',alignItems:'center',gap:6},puntosNum:{color:'#FFE566',fontSize:18,fontWeight:'900'},
  ...
  barraWrap:{width:'100%',height:5,backgroundColor:'rgba(255,255,255,0.15)',borderRadius:3,marginTop:4,overflow:'hidden'},barraFill:{height:'100%',borderRadius:3},
  ...
  navLabel:{fontSize:10,fontWeight:'800',color:'rgba(255,255,255,0.4)',letterSpacing:1,textTransform:'uppercase'},navLabelActivo:{color:'#FFE566'},
});
