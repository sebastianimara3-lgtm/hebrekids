...
const Burbuja = ({ letra, color, posX, esCertera, index, onPresionar, activa }) => {
  ...
  useEffect(() => {
    if (!activa) { posY.value = -100; opa.value = 1; escala.value = 1; return; }
    posY.value = withTiming(H + 150, { duration: 5000 + index * 400 });
...
const Particula = ({ x, y, color, index }) => {
  ...
  useEffect(() => {
    ...
    tx.value=withTiming(Math.cos(ang)*80,{duration:650});
    ty.value=withTiming(Math.sin(ang)*80+140,{duration:650});
...
const FeedbackFloat = ({ texto, color, visible }) => {
  ...
  useEffect(()=>{
    if(visible){
      opa.value=withSequence(withTiming(1,{duration:120}),withTiming(1,{duration:500}),withTiming(0,{duration:280}));
      posY.value=withTiming(-65,{duration:900});
...
export default function Level1Screen({ navigation }) {
  ...
  const [feedback,    setFeedback]    = useState({visible:false,texto:'',color:'#fff'});
  ...
  const generarRonda = useCallback(() => {
    ...
    setLetraActual(obj);
    escalaLetra.value = withSequence(withSpring(1.35,{damping:4}), withSpring(1.0,{damping:8}));
    ...
    setBurbujas(todas.map((item,i) => ({
      ...
      posX:      20 + (i % 3) * ((W - 100) / 2) + Math.random() * 20,
      color:     COLORES_BURBUJA[i % COLORES_BURBUJA.length],
      index:     i,
    })));
    setRonda(r => r+1);
  }, []);
  ...
  const manejarToque = useCallback(async (esAcierto, posX=W/2) => {
    ...
    if (esAcierto) {
      ...
      const parts = Array.from({length:12},(_,i) => ({
        id:`p_${Date.now()}_${i}`, x:posX, y:H*0.38,
        color:COLORES_BURBUJA[i%COLORES_BURBUJA.length], index:i,
      }));
      setParticulas(parts);
      timerRef.current = setTimeout(()=>setParticulas([]), 1100);
      ...
      setFeedback({visible:true, texto:'¡Ups! Seguí buscando 😅', color:'#FF6B6B'});
      timerRef.current = setTimeout(()=>setFeedback(f=>({...f,visible:false})), 900);

      // Resetear burbujas después de 2 segundos si no encuentra la correcta
      timerRef.current = setTimeout(()=>{
        setVidas(v => {
          const nv = v - 1;
          if (nv <= 0) setTimeout(()=>finalizarJuego(), 400);
          else generarRonda(); // Nueva ronda con las mismas vidas
          return nv;
        });
      }, 2000);
    }
    ...
    <View style={styles.contenedor}>
      ...
      <View style={styles.areaJuego} pointerEvents="box-none">
        {burbujas.map(b=>(
          <Burbuja
            key={b.id}
            letra={b.letra} color={b.color} posX={b.posX}
...
const styles = StyleSheet.create({
  ...
  objetivoWrap:{alignItems:'center',paddingVertical:14,backgroundColor:'rgba(0,0,0,0.25)'},
  objetivoLabel:{color:'rgba(255,255,255,0.55)',fontSize:12,fontWeight:'700',letterSpacing:1,textTransform:'uppercase',marginBottom:4},
  letraObjetivo:{color:'#FFE566',fontSize:68,fontWeight:'900',textAlign:'center',writingDirection:'rtl'},
  ...
  feedbackTexto:{fontSize:24,fontWeight:'900'},
  pantallaInicio:{position:'absolute',top:0,left:0,right:0,bottom:0,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(26,16,96,0.93)',zIndex:30,gap:14,paddingHorizontal:32},
