// src/screens/Level2Screen.js — Nivel 2 con fonética y alternancia hebreo/español
...
const Opcion=({item,tipo,seleccionada,correcta,incorrecta,usado,onPress,modo})=>{
  ...
  // Texto a mostrar en la columna izquierda según modo
  const textoIzq = modo===MODOS.FONETICA ? item.fonetica : modo===MODOS.ESPANOL ? item.español : item.hebreo;
  const esLatinIzq = modo!==MODOS.HEBREO;
    ...
    <Animated.View style={[styles.opcion,{backgroundColor:bg,borderColor:border,opacity:usado?0.35:1},estiloAnim]}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.8} style={styles.opcionTouch} disabled={usado||correcta}>
        {tipo==='imagen'
          ...
          :<Text style={esLatinIzq?styles.opcionFonetica:styles.opcionHebreo}>{textoIzq}</Text>
        }
      </TouchableOpacity>
    </Animated.View>
  );
...
export default function Level2Screen({navigation}){
  ...
  const generarRonda=useCallback(()=>{
    ...
    setSelIzq(null);setSelDer(null);setCorrectos(new Set());setIncorrectos(new Set());setRonda(r=>r+1);
  },[]);

  const verificarPar=useCallback(async(izq,der)=>{
    if(izq.id===der.id){
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      const n=new Set([...correctos,izq.id]);setCorrectos(n);setPuntos(p=>p+5);
      if(n.size>=4){setAciertos(a=>a+1);setPuntos(p=>p+10);setTimeout(()=>generarRonda(),800);}
    } else {
  ...
  const tocarIzq=item=>{if(correctos.has(item.id))return;const n=selIzq?.id===item.id?null:item;setSelIzq(n);if(n&&selDer)verificarPar(n,selDer);};
  const tocarDer=item=>{if(correctos.has(item.id))return;const n=selDer?.id===item.id?null:item;setSelDer(n);if(n&&selIzq)verificarPar(selIzq,n);};
  ...
  const finalizarJuego=useCallback(async()=>{
    ...
    Alert.alert('¡Nivel 2! 🧩',`Puntos: ${puntos}\nRondas: ${aciertos}`,[{text:'¡Otra vez!',onPress:iniciarJuego},{text:'Volver',onPress:()=>navigation.goBack()}]);
      ...
      {juegoActivo?(
        <>
          <View style={styles.instruccionWrap}>
            ...
            <Text style={styles.progreso}>{correctos.size}/4 pares • Ronda {ronda}</Text>
          </View>
          <View style={styles.areaJuego}>
            <View style={styles.columna}>
              <Text style={styles.colTitulo}>{modo===MODOS.ESPANOL?'Español':modo===MODOS.FONETICA?'Fonética':'Hebreo 🔤'}</Text>
              {colIzq.map(item=>(
                <Opcion key={`i${item.id}`} item={item} tipo="hebreo" modo={modo}
            ...
            <View style={styles.columna}>
              ...
              {colDer.map(item=>(
                <Opcion key={`d${item.id}`} item={item} tipo="imagen" modo={modo}
        ...
        <View style={styles.pantallaInicio}>
          ...
          <Text style={styles.inicioDesc}>Tocá la palabra y la imagen que le corresponde. ¡44 palabras!</Text>
...
const styles=StyleSheet.create({
  ...
  opcionEmoji:{fontSize:30},opcionEspanol:{color:'rgba(255,255,255,0.85)',fontSize:13,fontWeight:'700',textAlign:'center'},
  opcionHebreo:{color:'#fff',fontSize:24,fontWeight:'900',textAlign:'center',writingDirection:'rtl'},
  opcionFonetica:{color:'#fff',fontSize:16,fontWeight:'900',textAlign:'center'},
